...
namespace UIShared
{
	class UIE_ID_BlockRenderButton : public stackExternal::UI::UIElement, UIComponent_InputData
	{
	protected:
		...
		virtual bool OnEvent(unsigned int eventId, void* eventData) override
		{
			if (eventId == UISHAR__UICMD__SDL_EVENT && GetAttachedTree()->m_shouldRender == true) //is an event from sdl
			{
				...
				if (event->type == SDL_MOUSEBUTTONDOWN)
				{
					...
					{
						GetAttachedTree()->m_shouldRender = false;
						GetAttachedTree()->PassEventAsListUnhandleable(UISHAR__UICMD__RESET, nullptr);
		...
		virtual void LoadPropities(void* propertyContext, char* loadFromBuffer, unsigned int bufferLength) override
		{
			...
			//main box
			m_idOfBoxToRender = ((unsigned int)asset.GetSubObjectsByScope(0, "<idOfBoxToRender>", "</idOfBoxToRender>").GetContentsInt());


			//other boxes
			stackExternal::assets::AssetObjectContainer hideBoxesArray = asset.GetSubObjectsByScope(0, "<idsOfBoxesToHide>", "</idsOfBoxesToHide>");
			for (unsigned int i = 0; i < hideBoxesArray.GetSubObjectByArrayElementLength(); i++)
			{
				m_idsOfBoxesToHide.push_back((unsigned int)(hideBoxesArray.GetSubObjectByArrayElement(i).GetContentsInt()));
			}
	...
	public:
		...
		unsigned int m_idOfBoxToRender = 0;
		std::vector<unsigned int> m_idsOfBoxesToHide = std::vector<unsigned int>();
