...
namespace UIShared
{
	class SIMS_ID_DownloadButton : public stackExternal::UI::UIElement, UIComponent_InputData
	{
	protected:
		...
		virtual bool OnEvent(unsigned int eventId, void* eventData) override
		{
			if (eventId == UISHAR__UICMD__SDL_EVENT && GetAttachedTree()->m_shouldRender == true) //is an event from sdl
			{
				...
				if (event->type == SDL_MOUSEBUTTONDOWN && event->button.button == SDL_BUTTON_LEFT)
				{
					...
					{
						if (sfra::scrman::IsUpdateInProgress() == false && sfra::scrman::SafeToStartUpdateInstance() == true)
						{
