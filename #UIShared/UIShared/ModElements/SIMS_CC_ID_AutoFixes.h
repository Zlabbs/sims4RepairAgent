...
namespace UIShared
{
	class SIMS_CC_ID_AutoFixes : public stackExternal::UI::UIElement, UIComponent_ConnectionContext, UIComponent_InputData
	{
	private:
	...
	protected:
		...
		virtual void OnRender(void* renderingContext, bool renderDebug) override
		{
			...
			for (size_t i = 0; i < sfra::scrman::globals::n_masterScriptSet.size(); i++)
			{
				if (sfra::scrman::globals::n_masterScriptSet[i].inter_didAutoCheckScriptSayWeNeedToRun == true)
				{
					...
					m_titleText.SetText((SDL_Renderer*)renderingContext, sfra::scrman::globals::n_masterScriptSet[i].title);
					...
					m_descriptionText.SetText((SDL_Renderer*)renderingContext, sfra::scrman::globals::n_masterScriptSet[i].description);
