...
namespace UIShared
{
	class SIMS_CC_ID_ExecPath : public stackExternal::UI::UIElement, UIComponent_ConnectionContext, UIComponent_InputData
	{
	private:
		...
		void Enter()
		{
			...
			if (m_currentInput == "")
			{
				...
				{
					std::string mb = std::string(MB_CUR_MAX, '\0');
					int chunkSize = 0;
					wctomb_s(&chunkSize, &mb[0], MB_CUR_MAX, name[i]);

					for (int o = 0; o < chunkSize; o++)
					{
						m_currentInput = m_currentInput + mb[o];
					}
				}

				m_currentInput = m_currentInput + "\\Documents\\Electronic Arts\\The Sims 4";
			}

			sfra::scrman::globals::n_sims4Location = m_currentInput;
		}
	...
	protected:
		...
		virtual void OnRender(void* renderingContext, bool renderDebug) override
		{
			...
			if (m_lastFramesInput != currentOnScreenText)
			{
